...
namespace AdventOfCode2022
{
    public class Day7 : DayBase
    {
        ...
        private List<KeyValuePair<(string, string), long>> DirInfos(string name, int ContinueAt = 0)
        {
            ...
            var start = DirList.IndexOf($"$ cd {name}") + 2;
            var RelevantPart = DirList.GetRange(start, DirList.Count - start);
            ...
            var DirInfo = new KeyValuePair<(string, string), long>((name, "Directory"), DirSize);
            Result.Add(DirInfo);
            return Result;
        }
